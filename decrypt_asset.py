...
def decrypted_to_swf(data: bytes):
    """
    Converts the decrypted data to a swf file.
    """
...
def blowfish_decrypt(name: str, data: bytes):
    name_crc = c_uint32(0)
    ...
    size_tmp = c_uint32(int(len(data) / 16) % 16)
...
def decrypt_file(infile: Path, outdir: Optional[Path] = None):
    """
    ...
    outfile_raw = outdir / (name.split(".")[0] + ".swf.raw")
    with open(outfile_raw, 'wb') as f:
        f.write(blowfish_decrypted)
...
if __name__ == "__main__":
    ...
    decrypt_file(Path(args.infile), outdir)
